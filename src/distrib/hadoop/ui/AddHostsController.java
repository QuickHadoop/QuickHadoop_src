...
public class AddHostsController extends AnchorPane implements Initializable {
    ...
    /** 标题提示信息 */
    private static final String TITLE_INFO = Messages.getString("AddHostsController.title"); //$NON-NLS-1$
		...
		okBtn.setOnMouseClicked(new EventHandler<Event>() {
			...
			public void handle(Event event) {
				...
				if(!Util.ipIsValid(ip1) 
						|| !Util.ipIsValid(ip2)) {
					showErr(Messages.getString("AddHostsController.ip.invalid")); //$NON-NLS-1$
					return;
				}
				...
				if(!net1.equals(net2)) {
					showErr(Messages.getString("AddHostsController.net.notsame")); //$NON-NLS-1$
					return;
				}
				...
				if(start > end) {
					showErr(Messages.getString("AddHostsController.ip.err")); //$NON-NLS-1$
					return;
				}
				
				List<Runnable> runList = new ArrayList<Runnable>();
				for(int i = start; i <= end; i++) {
					final String ip = net1 + i;
					runList.add(new Runnable() {
						...
						public void run() {
							...
							Platform.runLater(new Runnable() {
							    @Override
							    public void run() {
							    	mainApp.getMainContr().addDataNode(ip);
							    }
							});
						}
					});
